...
<?php 
  ...
  { 
    ...
    if ($conn->connect_error) { 
        ...
        $sql = "SELECT * FROM status_manu WHERE sta_codigo = ".$id; 
    $result = mysqli_query($conn,$sql); 
    if ($result){ 
      $numreg = mysqli_num_rows ($result); 
    if ($numreg > 0){  
  ...
  {  
    ...
    if ($conn->connect_error) { 
        ...
        $insert = "INSERT INTO status_manu (
          sta_nome 
        ) values ( 
          '".$sta_nome."' 
        )"; 
        if (mysqli_query($conn,$insert) === FALSE) { 
          ...
          $sta_codigo = $PAGE->BuscaUltReg($conn,'status_manu','sta_codigo');  
          $registros[] = array( 
            'retorno' => 'OK', 
            'mensagem' => 'INSERIR status_manu COM SUCESSO! ', 
            'chave' => $sta_codigo 
          ); 
          echo json_encode($registros, JSON_PRETTY_PRINT); 
        } 
  ...
  {  
    ...
    if ($conn->connect_error) { 
        ...
        $insert = "UPDATE status_manu SET 
          sta_nome = '".$sta_nome."' 
        WHERE sta_codigo = ".$sta_codigo; 
        if (mysqli_query($conn,$insert) === FALSE) { 
          $registros[] = array( 
            'retorno' => 'ERRO', 
            'chave' => 0, 
          ...
          $registros[] = array( 
            'retorno' => 'OK', 
            'chave' => $sta_codigo, 
            'mensagem' => 'ATUALIZADO status_manu COM SUCESSO!',
            'items' => $items 
          ); 
          echo json_encode($registros, JSON_PRETTY_PRINT); 
        } 
  ...
  {  
    ...
    if ($conn->connect_error) { 
        ...
        $delete = "DELETE FROM status_manu 
        WHERE sta_codigo = ".$id; 
        if (mysqli_query($conn,$delete) === FALSE) { 
          ...
          $registros[] = array( 
            'retorno' => 'OK', 
            'mensagem' => 'EXCLUIDO REGISTRO DE status_manu COM SUCESSO! ' 
          ); 
        } 
        echo json_encode($registros, JSON_PRETTY_PRINT); 
  } 
