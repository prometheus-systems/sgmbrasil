...
<?php 
  ...
  if ($conn->connect_error) {   
      ...
      $nome = 'upload/ID'.$_POST['usu_codigo'].'/'.$_FILES['maq_imagem']['name'];  
      $insert = "UPDATE maquinas SET maq_imagem = '".$nome."'  
                  WHERE maq_codigo = ".$_POST['maq_codigo']; 
...
if (!empty($_FILES)){ 
      ...
      if (!$_FILES['maq_imagem']['name']){   
        ...
        if (file_exists($local)) {  
            ...
            $arquivo = 'ID'.$idusuario.'_'.$_FILES['maq_imagem']['the_name_of_the_image_field']; 
        }   
      }else{    
        if (!file_exists($local)) { 
          mkdir($local, 0755, true); 
        ...
        if (file_exists($local)) {  
          $path = $local.'/'.$_FILES['maq_imagem']['name']; 
          $arquivo = $_FILES['maq_imagem']['tmp_name']; 
        }else{ 
          $local  = 'upload';  
          $path = $local.'/'.$_FILES['maq_imagem']['name']; 
          rename ($path, 'ID'.$idusuario.'_'.$path); 
          $arquivo = 'ID'.$idusuario.'_'.$_FILES['maq_imagem']['tmp_name'];
        }   
      } 
