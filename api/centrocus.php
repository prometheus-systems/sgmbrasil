...
<?php 
  ...
  { 
    ...
    if ($conn->connect_error) { 
        ...
        $sql = "SELECT * FROM centrocus WHERE cen_codigo = ".$id; 
    $result = mysqli_query($conn,$sql); 
    if ($result){ 
      $numreg = mysqli_num_rows ($result); 
    if ($numreg > 0){  
  ...
  {  
    ...
    if ($conn->connect_error) { 
        ...
        if (mysqli_query($conn,$insert) === FALSE) { 
          ...
          $cen_codigo = $PAGE->BuscaUltReg($conn,'centrocus','cen_codigo');  
          $registros[] = array( 
            'retorno' => 'OK', 
            'mensagem' => 'INSERIR centrocus COM SUCESSO! ', 
            'chave' => $cen_codigo 
          ); 
          echo json_encode($registros, JSON_PRETTY_PRINT); 
        } 
  ...
  {  
    ...
    if ($conn->connect_error) { 
        ...
        $insert = "UPDATE centrocus SET 
          cen_nome = '".$cen_nome."' 
        WHERE cen_codigo = ".$cen_codigo; 
        if (mysqli_query($conn,$insert) === FALSE) { 
          $registros[] = array( 
            'retorno' => 'ERRO', 
            'chave' => 0, 
          ...
          $registros[] = array( 
            'retorno' => 'OK', 
            'chave' => $cen_codigo, 
            'mensagem' => 'ATUALIZADO centrocus COM SUCESSO!',
            'items' => $items 
          ); 
          echo json_encode($registros, JSON_PRETTY_PRINT); 
        } 
  ...
  {  
    ...
    if ($conn->connect_error) { 
        ...
        if (mysqli_query($conn,$delete) === FALSE) { 
          ...
          $registros[] = array( 
            'retorno' => 'OK', 
            'mensagem' => 'EXCLUIDO REGISTRO DE centrocus COM SUCESSO! ' 
          ); 
        } 
        echo json_encode($registros, JSON_PRETTY_PRINT); 
  } 
