...
<?php 
  ...
  { 
    ...
    if ($conn->connect_error) { 
        ...
        $sql = "SELECT * FROM espera WHERE esp_codigo = ".$id; 
    $result = mysqli_query($conn,$sql); 
    if ($result){ 
      $numreg = mysqli_num_rows ($result); 
    if ($numreg > 0){  
  ...
  {  
    ...
    if ($conn->connect_error) { 
        ...
        if (mysqli_query($conn,$insert) === FALSE) { 
          ...
          $esp_codigo = $PAGE->BuscaUltReg($conn,'espera','esp_codigo');  
          $registros[] = array( 
            'retorno' => 'OK', 
            'mensagem' => 'INSERIR espera COM SUCESSO! ', 
            'chave' => $esp_codigo 
          ); 
          echo json_encode($registros, JSON_PRETTY_PRINT); 
        } 
    ...
    if ($conn->connect_error) { 
        ...
        if (mysqli_query($conn,$insert) === FALSE) { 
          ...
          $registros[] = array( 
            'retorno' => 'OK', 
            'chave' => $esp_codigo, 
            'mensagem' => 'ATUALIZADO espera COM SUCESSO!',
            'items' => $items 
          ); 
          echo json_encode($registros, JSON_PRETTY_PRINT); 
        } 
  ...
  {  
    ...
    if ($conn->connect_error) { 
        ...
        if (mysqli_query($conn,$delete) === FALSE) { 
          ...
          $registros[] = array( 
            'retorno' => 'OK', 
            'mensagem' => 'EXCLUIDO REGISTRO DE espera COM SUCESSO! ' 
          ); 
        } 
        echo json_encode($registros, JSON_PRETTY_PRINT); 
  } 
