  ...
  if ($conn->connect_error) {   
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && empty($_POST)) 
     ...
     $PAGE->imp($_POST);   
      ...
      $nome = 'upload/ID'.$_POST['usu_codigo'].'/'.$_FILES['pec_imagem']['name'];  
      echo $insert = "UPDATE pecas SET pec_imagem = '".$nome."'  
                  WHERE pec_codigo = ".$_POST['pec_codigo']; 
    ...
    echo 'tem arquivo';
      ...
      if (!$_FILES['pec_imagem']['name']){   
        ...
        if (file_exists($local)) {  
            ...
            $arquivo = 'ID'.$idusuario.'_'.$_FILES['pec_imagem']['the_name_of_the_image_field']; 
        }   
      }else{    
        if (!file_exists($local)) { 
          mkdir($local, 0755, true); 
        ...
        if (file_exists($local)) {  
          $path = $local.'/'.$_FILES['pec_imagem']['name']; 
          $arquivo = $_FILES['pec_imagem']['tmp_name']; 
        }else{ 
          $local  = 'upload';  
          $path = $local.'/'.$_FILES['pec_imagem']['name']; 
          rename ($path, 'ID'.$idusuario.'_'.$path); 
          $arquivo = 'ID'.$idusuario.'_'.$_FILES['pec_imagem']['tmp_name'];
        }   
      } 
